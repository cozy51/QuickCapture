...
public sealed class ImageViewport : FrameworkElement, IDisposable
{
    ...
    public bool IsInteracting => panning || Highlighter.IsDrawing;
    ...
    internal static DragAction ResolveDrag(bool highlighting, ModifierKeys modifiers, bool space) =>
        space || (modifiers & ModifierKeys.Alt) != 0 ? DragAction.PanImage : highlighting || (modifiers & ModifierKeys.Control) != 0 ? DragAction.Highlight : DragAction.MoveWindow;
    ...
    public void ToggleHighlighter() { CancelInteraction(); IsHighlighting = !IsHighlighting; UpdateCursor(); Refresh(); }
    public void ChangeWidth(int direction) { Highlighter.ChangeWidth(direction); Refresh(); }
    public void UpdateCursor() => Cursor = Highlighter.IsDrawing ? Cursors.Pen : panning ? Cursors.Hand : ResolveDrag(IsHighlighting, Keyboard.Modifiers, Keyboard.IsKeyDown(Key.Space)) switch
    {
    ...
    protected override void OnRender(DrawingContext dc)
    {
        ...
        DrawingLayer.Render(dc, document, Highlighter.Preview);
        dc.Pop();
    }
    protected override void OnMouseWheel(MouseWheelEventArgs e)
    {
        ...
        if (IsHighlighting && (Keyboard.Modifiers & ModifierKeys.Control) != 0) { ChangeWidth(Math.Sign(e.Delta)); return; }
    ...
    {
        Focus();
        var action = ResolveDrag(IsHighlighting, Keyboard.Modifiers, Keyboard.IsKeyDown(Key.Space));
        ...
        {
            var point = Zoom.ToImage(e.GetPosition(this));
            if (new Rect(0, 0, document.Width, document.Height).Contains(point)) { Highlighter.Begin(point); CaptureMouse(); Refresh(); }
            e.Handled = true; return;
        }
        panning = true; previous = e.GetPosition(this); CaptureMouse(); e.Handled = true;
    }
    protected override void OnMouseMove(MouseEventArgs e)
    {
        ...
        {
            Highlighter.Add(Zoom.ToImage(e.GetPosition(this)), Math.Max(0.2, 0.75 / Zoom.ViewScale));
    ...
    protected override void OnMouseLeftButtonUp(MouseButtonEventArgs e)
    {
        if (Highlighter.IsDrawing)
        {
            Highlighter.Add(Zoom.ToImage(e.GetPosition(this)), 0.01);
            var stroke = Highlighter.Finish();
            if (stroke != null) document.Add(stroke);
        }
        panning = false; ReleaseMouseCapture(); e.Handled = true; UpdateCursor(); Refresh();
    }
    protected override void OnLostMouseCapture(MouseEventArgs e) { panning = false; Highlighter.Cancel(); InvalidateVisual(); base.OnLostMouseCapture(e); }
    public void CancelInteraction() { panning = false; Highlighter.Cancel(); ReleaseMouseCapture(); Refresh(); }
    public void CancelMode() { CancelInteraction(); IsHighlighting = false; UpdateCursor(); Refresh(); }
    public void Dispose() { document.Changed -= Refresh; CancelMode(); ViewChanged = null; ZoomSizeChanged = null; }
}
