...
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。**日本語入力（IME）**に対応します。入力欄を開くと**IMEを自動でON**にします。入力欄の右に**「あ / A」の切替ボタン**を置いているので、クリックで日本語入力と英数を切り替えられます（**半角/全角キー**、**F2**、**Ctrl+Space**でも切り替わります）。環境によってモードキーがIMEに届かないことがあるため、入力欄が開いている間だけ**このスレッド限定のキーボードフック**を置き、WPFやIMEより早い段階でモードキーを受け取ります。押した回数どおりにモードが入れ替わり、どの操作で切り替わったか（キー／ボタン）もステータスに表示します。切り替えた状態は次に開く入力欄にも引き継ぎます。入力中は変換候補が隠れないよう最前面表示を一時的に解除します。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。
