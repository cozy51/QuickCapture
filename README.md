...
時刻は**22 DIPの文字で中央揃え**です。番号・年月日と同じ1行で上下中央に揃え、上部の帯は34 DIPに抑えています。年月日は閉じるボタンの左に右寄せで置きます。狭いウィンドウでは番号や閉じるボタンと重ならないよう、時刻だけ縮小し、必要なら総枚数・年月日・「3秒で閉じる」表示を省略します。
...
記録として残したいときは、トレイ → **設定… →「コピー・PNG保存画像に上部の情報バーを含める」**をONにします。画像の上に、画面と同じ**番号／枚数・撮影時刻・撮影年月日**の黒い帯（34px）を付けて出力します。自動コピー・Ctrl+C・PNG保存すべてが対象で、出力の高さは34px増えます（850×420 → 850×454px、外枠と併用すると852×456px）。閉じるボタンは操作用の部品なので出力には入れません。

その画像だけ切り替えたい場合は、画像を右クリックして**「コピー・保存に上部バーを含める」**をON/OFFします。この操作は開いているその画像にだけ効き、次回以降のキャプチャには影響しません。標準はOFFで、これまでどおり画像だけを出力します。
