...
- **緑の破線**：画面に残す画像。
- 一時画像を残したい場合は、その画像を右クリックして**「この画像から自動終了をやめる」**を選びます。その画像のカウントダウンをその場で止め、**以降に撮影する画像も残す設定として保存**します（「この画像から3秒で閉じる」をOFFにするのと同じ動作です）。

残す画像と一時的な画像を混在できます。描画やパンで期限は延長しないため、編集を続けたい一時画像は「この画像から自動終了をやめる」で保持に切り替えてください。PNG保存ダイアログを開いている間だけ自動終了を止め、ダイアログを閉じてから3秒で終了します。
...
| この画像と以降のキャプチャを残す | 右クリック → この画像から自動終了をやめる |
...
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。**日本語入力（IME）**に対応します。入力欄を開くと**IMEを自動でON**にし、**半角/全角キー（またはCtrl+Space）で日本語入力と英数を切り替え**られます。切り替えた状態は次に開く入力欄にも引き継ぎます。WPF経由ではモードキーがIMEに届かない環境があるため、キーをアプリ側で受け取ってIMEを直接切り替えています。入力中は変換候補が隠れないよう最前面表示を一時的に解除します。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。
