...
| 色を変える | 上部ツールバーの色丸／右クリック → 描画ツール → 色 |
...
上部のツールバーに**蛍光ペン（H）・ペン（✎）・テキスト（T）**と**色丸**が並びます。色丸を押すと、**その時に選んでいるツールの色**が変わります（ツール未選択なら蛍光ペンを選びます）。選んだ色と線幅は**設定ファイルへ保存し、次回以降のキャプチャにも引き継ぎます**。
...
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。**日本語入力（IME）**に対応し、入力中は変換候補が隠れないよう最前面表示を一時的に解除します。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。
  ...
  - 選択中の文字は **`[`／`]`（またはCtrl＋ホイール）で大きさ**、**色丸で色**を変更できます。移動・大きさ・色の変更はすべてUndoできます。
