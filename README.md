...
通常のウィンドウ移動は**左ドラッグ**です。描画ツール（蛍光ペン・ペン）がONのときは左ドラッグで描画するため、移動にはツールを解除するか上端ツールバーの左端のつまみを使います。外周をドラッグするとサイズ変更できます。右クリックでも主要な操作を使えます。
...
画像を右クリックして**「この画像から3秒で閉じる」**をONにすると、**その画像がその場で3秒の自動終了に切り替わり**、以降に撮影する画像にも同じモードが適用されます。画像上の**Tキー**でも同じ設定を切り替えます。OFFにした場合も、その画像の自動終了をその場で解除し、以降の撮影は画面に残します。既に開いている他の画像のモードは変わりません（トレイの**「キャプチャを3秒で閉じる」**は次に撮影する画像から適用されます）。設定は保存され、OFFに戻すまで連続して適用します。

**「連続モード（3秒で閉じる＋日時情報）」**は、この「この画像から3秒で閉じる」と「コピー対象に上部のカウンターと日時情報を含める」を**ひとつのスイッチでまとめて切り替える**項目です。右クリックメニューの「この画像から3秒で閉じる」のすぐ下にあり、クリックするたびに**両方まとめてON／OFF**を繰り返します。**両方がONのときだけチェックが付く**ため、片方だけONの状態でクリックすると足りない方もONに揃えます。よく使う項目なので、太字に加えて**記録帯と同じ濃い青（#153E76）の文字色**で強調しています。ONにした直後は3秒のカウントダウンを始め、あわせてその画像を帯付きで自動再コピーします。

自動終了する画像には、**残り秒数を大きな数字**で画像の中央に表示します。この数字はコピー・保存する画像には入りません。いつものCtrl+Shift+Rやトレイ左クリックで撮影できます。範囲選択が自動で再開する方式ではありません。元画像はこれまでどおり範囲確定直後に自動コピーし、閉じた後も最後にコピーした画像を貼り付けられます。

- **オレンジ（#FFA43E）の点線**：3秒で閉じる画像。
- **緑の破線**：画面に残す画像。
- 一時画像を残したい場合は、その画像を右クリックして**「この画像から3秒で閉じる」のチェックを外します**。その画像のカウントダウンをその場で止め、**以降に撮影する画像も残す設定として保存**します。

残す画像と一時的な画像を混在できます。描画やパンで期限は延長しないため、編集を続けたい一時画像は「この画像から3秒で閉じる」のチェックを外して保持に切り替えてください。PNG保存ダイアログを開いている間だけ自動終了を止め、ダイアログを閉じてから3秒で終了します。
...
| パン | Alt＋左ドラッグ／Space＋左ドラッグ（描画中も有効） |
| ウィンドウ移動 | 左ドラッグ（描画ツールOFF時）／ツールバー左端をドラッグ |
| 100%表示 | Ctrl+0／1（テンキーも対応） |
| ウィンドウにFit | F |
| 100%とFitの切替 | ダブルクリック（蛍光ペンOFF時） |
| 蛍光ペンON/OFF | H |
| 一時的に蛍光ペンを使う | Ctrl＋左ドラッグ（ツールのON/OFFを変更しません） |
| 一時的にペンを使う | Shift＋左ドラッグ（ツールのON/OFFを変更しません） |
| ペン（不透明な線）のON/OFF | P／上部ツールバーの✎／右クリック → 描画ツール → ペン |
| テキスト（文字入れ）のON/OFF | X／上部ツールバーのT／右クリック → 描画ツール → テキスト |
| 置いた文字を書き直す | テキストツール中に文字をダブルクリック |
| 線を細く／太くする・文字を小さく／大きくする | `[`／`]`、またはツール選択中にCtrl＋ホイール |
| 色を変える | 上部ツールバーの色プルダウン／右クリック → 描画ツール → 色 |
...
| この画像と以降のキャプチャの3秒自動終了をON/OFF | T／右クリック → この画像から3秒で閉じる |
| コピー・保存に上部の帯を含める | 右クリック → **コピー対象に上部のカウンターと日時情報を含める** |
| 3秒自動終了と上部の帯をまとめてON/OFF | 右クリック → **連続モード（3秒で閉じる＋日時情報）** |
...
上部のツールバーに**蛍光ペン（H）・ペン（✎）・テキスト（T）**と**色のプルダウン**が並びます。プルダウンのボタンには**今のツールの色**が丸で表示され、押すと色の一覧が下に開きます。選ぶと、**その時に選んでいるツールの色**が変わります（ツール未選択なら蛍光ペンを選びます）。選んだ色と線幅は**設定ファイルへ保存し、次回以降のキャプチャにも引き継ぎます**。

- **蛍光ペン**（H／Ctrl＋ドラッグ）: 半透明でマーキングします。初期値は緑（#68E675）・20px・不透明度45%。
- **ペン**（P／**Shift＋ドラッグ**）: 不透明な線で書き込みます。初期値は**青（#2F7BF6）**・4px。Shift＋左ドラッグなら、ツールを切り替えずにその操作の間だけペンで書けます。
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。**日本語入力（IME）**に対応します。入力欄を開くと**IMEを自動でON**にします。入力欄の右に**「あ / A」の切替ボタン**を置いているので、クリックで日本語入力と英数を切り替えられます（**半角/全角キー**、**F2**、**Ctrl+Space**でも切り替わります）。環境によってモードキーがIMEに届かないことがあるため、入力欄が開いている間だけキーボードフックを置き、WPFやIMEより早い段階でモードキーを受け取ります。フックは**QuickCaptureのウィンドウが前面にあり、入力欄にフォーカスがある間だけ**反応し、それ以外のキーは必ずそのまま次へ渡します。ウィンドウが非アクティブになるとシステム全体を見るフックは即座に外すため、**他のアプリの半角/全角キーには影響しません**。押した回数どおりにモードが入れ替わり、どの操作で切り替わったか（キー／ボタン）もステータスに表示します。切り替えた状態は次に開く入力欄にも引き継ぎます。入力中は変換候補が隠れないよう最前面表示を一時的に解除します。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。
  - テキストツール中は、置いた文字が**破線の枠**で示されます。**クリックで選択**すると枠が実線になり、**ドラッグで移動**できます。**ダブルクリックで書き直し**（元の色・大きさ・位置のまま）でき、**空にして確定すると消えます**。書き直しも削除もUndoできます。
  - 選択中の文字は **`[`／`]`（またはCtrl＋ホイール）で大きさ**、**色プルダウンで色**を変更できます。移動・大きさ・色の変更はすべてUndoできます。
  - 文字色と文字サイズは**保存**し、次回以降のキャプチャに引き継ぎます。
- 色は青・赤・黄色・緑・水色・ピンク・黒から選べます。線幅は元画像のピクセル単位です。

描画中のホイールは無効にして、線の位置が跳ばないようにしています。ドラッグ中断時の未確定線は取り消します。描画クリアもUndoでき、履歴は直近100操作までです。

Ctrl＋左ドラッグは押し始めた時点で描画の操作になり、途中でCtrlを離してもマウスを離すまで線を描きます。次の通常の左ドラッグはウィンドウ移動に戻ります（HがONなら描画を継続）。Alt／Spaceを同時に押した場合はパンを優先します。
...
記録として残したいときは、トレイ → **設定… →「コピー・PNG保存画像に上部のカウンターと日時情報を含める」**をONにします。画像の上に、画面と同じ**番号／枚数・撮影時刻・撮影年月日**の帯（34px）を付けて出力します。自動コピー・Ctrl+C・PNG保存すべてが対象で、出力の高さは34px増えます（850×420 → 850×454px、外枠と併用すると852×456px）。閉じるボタンは操作用の部品なので出力には入れません。

画像を右クリックして**「コピー対象に上部のカウンターと日時情報を含める」**でも切り替えられます（よく使う項目なので、3秒自動終了の項目とともにメニューでは太字にしています）。この操作は**その場で反映**し、**開いているすべての画像**と**次回以降のキャプチャ**にも適用したうえで設定ファイルへ保存します。切り替えた直後にその画像を**自動で再コピー**するため、クリップボードの中身も新しい設定に揃います（このときは「コピー後に閉じる」設定でも画像は閉じません）。標準はOFFで、これまでどおり画像だけを出力します。3秒自動終了と一緒に使うことが多いため、両方を一度に切り替える**「連続モード（3秒で閉じる＋日時情報）」**も同じメニューに用意しています。

ONの間は上部の帯の背景を**濃い青（#153E76）**にして、記録付きで出力する状態だとひと目で分かるようにしています。あわせて**緑の破線枠を帯の上まで広げ**、帯ごと囲んだ表示にするので、出力される範囲がそのまま枠の内側になります。OFFに戻すと通常の黒い帯（#181B20）と、画像だけを囲む枠に戻ります。出力される帯も同じ濃い青です。
...
PerMonitorV2のマニフェストに加えて起動時にコードでもPerMonitorV2を設定し（exe以外の起動方法でも同じ挙動にするため）、画面取得・矩形選択・モニター配置・ウィンドウ枠（破線・上部の帯・閉じるボタン）・最小サイズは物理ピクセルで統一しています。表示倍率の異なるモニターへ移動しても枠の太さや帯の高さは変わりません。キャプチャ直後のウィンドウは、**表示する画像が撮影した画素の真上に重なる位置**へ置きます（上部の帯と枠の分だけ左上へずらして配置します）。ウィンドウの外形もWPFのレイアウト寸法（DIP）ではなく画素数で決め、WPF側のWidth／Heightも同じ画素数に合わせています。倍率が変わってもウィンドウは画像にぴったり合ったままで、画像の周囲に黒い余白は出ません。余分な余白が生じた場合はレイアウト後に切り詰めます（ユーザーがドラッグでサイズを変えた場合は尊重します）。モニターごとに別のオーバーレイを置き、同じグローバル選択矩形をそれぞれのWPF表示サイズへ変換します。ズーム・パン・蛍光ペンには共通の変換を使います。表示構成が変更された場合は選択を中止します。
