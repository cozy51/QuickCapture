...
参照画像のウィンドウ枠は明るい緑（#42DD75）の破線、太さ3 DIPです（100%表示で3px相当）。猫の配置をカーソル右下のすぐ近くへ寄せています。右クリックメニューは先頭と末尾の両方に「閉じる」を用意しています。

画像の破線枠の上に、**大きな番号／表示中の枚数**と**撮影時刻（HH:mm:ss）**を表示します。番号は表示中の画像を撮影順に並べた順番で、閉じる・最小化・再表示に合わせて振り直します。画像を手前へ出しても順番は変わりません。時刻は撮影時点の値で固定し、この表示部分をドラッグしてウィンドウを移動できます。番号・時刻はコピーや保存画像に入りません。

時刻は**22 DIPの文字で中央揃え**です。番号と同じ1行で上下中央に揃え、上部の帯は34 DIPに抑えています。狭いウィンドウでは番号と重ならないよう、時刻だけ縮小し、必要なら総枚数の表示を省略します。
...
アプリを実行するときは、**exeまたは`dotnet run`を使用**してください。`dotnet QuickCapture.dll`による起動はホスト側のDPI設定になるため使用しません。
...
ショートカットは保存後すぐ反映します。画像・描画・コピーの初期設定は、以降に開く画像ウィンドウに適用します。外枠設定は、既に開いている画像にも次のコピー・保存から反映します。各画像の右クリックで変更する色や最前面状態は、その画像だけのものです。不透明度の変更は終了中にJSONを編集してください。設定は検証後に一時ファイルから置換し、破損した設定は保持したまま初期値で起動します。
...
src/QuickCapture/
  ...
  Controls/
    ...
    CaptureFrame.cs              明るい緑の破線枠、番号・撮影時刻
  Windows/
...
PerMonitorV2のマニフェストを使用し、画面取得・矩形選択・モニター配置は物理ピクセルで統一しています。モニターごとに別のオーバーレイを置き、同じグローバル選択矩形をそれぞれのWPF表示サイズへ変換します。ズーム・パン・蛍光ペンには共通の変換を使います。表示構成が変更された場合は選択を中止します。
