...
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。**日本語入力（IME）**に対応し、入力中は変換候補が隠れないよう最前面表示を一時的に解除します。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。
