...
- **ペン**（P）: 不透明な線で書き込みます。初期値は**青（#2F7BF6）**・4px。
- **テキスト**（X）: 画像をクリックするとその場に入力欄が出ます。**Enterで確定、Shift+Enterで改行、Escで取り消し**。日本語入力（IME）も使えます。初期の文字色は**青（#2F7BF6）**・24px、白い縁取りを付けて画像の上でも読めるようにしています。**選んだ文字色も保存**し、次回以降のキャプチャに引き継ぎます。**置いた文字はテキストツール中にドラッグして移動**でき、移動もUndoできます。
